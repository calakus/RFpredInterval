...
void registerCustomFunctions() {
  ...
  registerThis (&l1split, REGR_FAM, 2);
  registerThis (&spisplit, REGR_FAM, 3);
}
                ...
                unsigned int  featureCount)
{
  ...
  unsigned int i, cur1 = 0, cur2 = 0;
  double f1 = 0.0, f2 = 0.0, dis = 0.0, curp = 0.0;
  double delta = 0.0;
  ...
  double left[leftSize], right[rghtSize];
  ...
  for (i = 1; i <= n; i++) {
    if (membership[i] == LEFT) {
      left[leftSize] = response[i];
      leftSize ++;
    }
    else {
      right[rghtSize] = response[i];
      rghtSize ++;
    }
  }

  qsort (left, leftSize, sizeof(left[0]), compare);
  qsort (right, rghtSize, sizeof(right[0]), compare);

  while (f1 + f2 < 2) {
    if (cur1 >= leftSize) {
      dis = dis + (right[cur2] - curp) * fabs(f1 - f2);
      f2 = f2 + 1.0/rghtSize;
      curp = right[cur2];
      cur2 ++;
    }
    else if (cur2 >= rghtSize) {
      dis = dis + (left[cur1] - curp) * fabs(f1 - f2);
      f1 = f1 + 1.0/leftSize;
      curp = left[cur1];
      cur1 ++;
    }
    else if (left[cur1] > right[cur2]) {
      dis = dis + (right[cur2] - curp) * fabs(f1 - f2);
      f2 = f2 + 1.0/rghtSize;
      curp = right[cur2];
      cur2 ++;
  ...
  delta = leftSize * rghtSize * dis;

  return delta;
    
}

double spisplit (unsigned int n,
                 char        *membership,
                 double      *time,
                 double      *event,
                 ...
                 unsigned int  featureCount)
{
  ...
  unsigned int i, m1 = 0, m2 = 0, leftSize = 0, rghtSize = 0;
  double minlall = 0.0, bestle1 = 0.0, bestle2 = 0.0, le = 0.0;
  double delta = 0.0;
  ...
  m1 = ceil(0.95 * leftSize);
  m2 = ceil(0.95 * rghtSize);

  bestle1 = left[leftSize - 1] - left[0];
  bestle2 = right[rghtSize - 1] - right[0];

  if (leftSize > m1) {
    for (int i = 0; i < (leftSize - m1); ++i) {
      le = left[i + m1 - 1] - left[i];
      if (le < bestle1) {
        bestle1 = le;
      }
    }
  ...
  if (rghtSize > m2) {
    for (int i = 0; i < (rghtSize - m2); ++i) {
      le = right[i + m2 - 1] - right[i];
      if (le < bestle2) {
        bestle2 = le;
      }
    }
  ...
  minlall = MAX(left[leftSize - 1], right[rghtSize - 1]) - MIN(left[0], right[0]);

  delta = (n * minlall) - (leftSize * bestle1 + rghtSize * bestle2);
